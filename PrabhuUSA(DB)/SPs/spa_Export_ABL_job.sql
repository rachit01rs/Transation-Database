/****** Object:  StoredProcedure [dbo].[spa_Export_ABL_job]    Script Date: 09/23/2014 12:11:55 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spa_Export_ABL_job]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spa_Export_ABL_job]
GO
...
--spa_Export_ABL_job '20100072',NULL,'NBLADMIN01','30108381','moin','a','NationalBank'  
CREATE PROCEDURE [dbo].[spa_Export_ABL_job]   
...
--if @ditital_id is null  
 ...
 SET @job_name = 'spa_Export_ABL' + @process_id  
 SET @spa = 'spa_Export_ABL   ' + case when @agent_id is null then ' Null '  else  '''' + @agent_id  +  ''''  end +',
