USE [PrabhuUsa]
GO
/****** Object:  StoredProcedure [dbo].[spa_moneysend_main_V2]    Script Date: 02/11/2015 02:02:28 ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [dbo].[spa_moneysend_main_V2]                        
@flag char(1),                        
@refno varchar(15),                        
...
@sPaymentReceivedType varchar(50)=NULL, -- For knowing the sender deposit type                     
@sCheque_bank VARCHAR(200)=NULL, -- For knowing the sender deposit type                 
@sChequeno VARCHAR (100)=NULL, -- For knowing the sender deposit type                 
...
declare @rbankname varchar(150),@rbankbranch varchar(150),@receiveagentid varchar(50),@receivectype varchar(50),                        
@payout_country varchar(100),@branch_group varchar(500),@super_agent_id varchar(50)                        
                        
if @rbankid is not null                        
begin            
 select @receiveagentid=a.agentcode,@rbankname=a.companyName,@rBankBranch=b.branch,@branch_group=b.branch_group,@super_agent_id=a.super_agent_id   from                        
 agentdetail a WITH(NOLOCK) join agentbranchdetail b WITH(NOLOCK) on a.agentcode=b.agentcode                        
 where agent_branch_code=@rbankid                        
end                        
else                        
begin                        
 select @receiveagentid=agentcode,@rbankname=companyName,@super_agent_id=super_agent_id from agentdetail WITH(NOLOCK) where agentcode=@payout_agent_id                        
end                        
...
begin                        
 ...
 return                        
            ...
            --return             
select @receivectype=currencyType,@payout_country=country from agentdetail WITH(NOLOCK) where agentcode=@payout_agent_id                        
...
if (@paymenttype='NEFT' or @paymenttype='RTGS' ) AND @c2c_receiver_code is not null                  
begin                  
...
-- end                       
 ...
 and exists (select bankcode from session_deposit_log WITH(NOLOCK)                         
 where bankcode=@cash_ledger_id and session_id=@session_id )  -- BANK DEPOSIT WITH CASH ID                        
 begin                        
...
begin transaction                        
 ...
 begin                          
  set @cust_sno= ident_current('customerdetail')+IDENT_INCR('customerdetail')                       
...
if @SSN_Card_ID is not null 
begin
	if exists(select sno from customerdetail where SSN_Card_ID=replace(@SSN_Card_ID,' ','') and customerid <> isnull(@customerid,'1'))
	begin
			 select 'ERROR','7001','SSN ID '+ @SSN_Card_ID+' found duplicate'                  
			 return  
	end 
end                 
 if @customer_type='i'                        
 ...
 if exists (SELECT 'x' FROM customerDetail  WITH(NOLOCK)     
		 WHERE senderpassport=@senderpassport AND senderFax=@txtsPassport_type    
			AND CASE WHEN @txtsPassport_type = 'passport' THEN SenderNativeCountry    
		 ELSE SenderCountry  END = CASE WHEN @txtsPassport_type = 'passport' THEN @sender_native_country    
		 ELSE @SenderCountry  END )                  
 begin                  
...
ID_Issue_date,SSN_Card_ID,Date_of_Birth,Sender_State,IssueAuthority,create_ts,FreeSMS,picture_id_type,SenderZipCode)                         
...
@SSN_Card_ID,@Date_of_Birth,@Sender_State,@IssueAuthority,getdate(),@FreeSMS,@PictureIdType,@sendersalary)             
  
  SELECT @cust_sno=sno FROM customerDetail cd with (nolock) WHERE cd.CustomerId=@customerid                    
  UPDATE customer_doc SET CustomerId=@customerid WHERE CustomerId=@session_id                         
 ...
 begin        
  ...
  select @cust_sno=sno,@senderpassport_old=senderPassport,@txtsPassport_type_old=senderFax from customerdetail with (nolock) where customerid=@customerid     
  ...
  if @cust_sno is not null                        
   begin                        
    ...
    sender_fax_no=@sender_fax_no,
	ID_Issue_date=@ID_Issue_date,      
    SSN_Card_ID=@SSN_Card_ID,                        
    Date_of_Birth=@Date_of_Birth,      
    Sender_State=@Sender_State,              
    IssueAuthority=@IssueAuthority,FreeSMS = @FreeSMS,      
    update_ts=GETDATE(),picture_id_type=@PictureIdType,
	SenderZipCode=@sendersalary            
    where sno=@cust_sno       
    --PRINT(@cust_sno)        
    --   PRINT(@Date_of_Birth)         
    --    PRINT(@ID_Issue_date)               
...
set @agent_ex_gain=round(@agent_ex_gain,2,1)                    
set @check_agent_ex_gain=round(@check_agent_ex_gain,2,1)                    
...
ELSE if @payout_agent_id='10000004' ---  MUSLIM COMMERCIAL BANK        
 ...
 if exists(select ref_sno from tbl_refno with (nolock) where refno=@enc_refno)                        
 begin                        
 ...
 IF EXISTS(SELECT 'x' FROM temp_compliance_log tcl WITH(NOLOCK) WHERE tcl.process_id=@session_id)              
 BEGIN               
    SET @compliance_flag='y'                        
      SET @compliance_sys_msg= case when @compliance_sys_msg is null then 'Transaction is in Compliance'                         
            else @compliance_sys_msg + ' <br>Transaction is in Compliance' End               
 END                       
 --ADDED FOR INTEGRATED AGNET SAVE                         
declare @status varchar(50)                 
set @status='Un-Paid'                          
if exists (select agentcode from tbl_integrated_agents WITH(NOLOCK) 
where (agentcode=@payout_agent_id or agentcode=@super_agent_id))                          
begin                          
 set @status='Post'                          
...
--INTEGRATED AGENT END                         
 ...
 compliance_flag,compliance_sys_msg,agent_receiverCommission,              
 [sPaymentReceivedType],[sCheque_bank],[sChequeno] ,IssueAuthority,payout_send_agent_id,ben_bank_branch_id,FreeSMS,picture_id_type,
customer_category_id,agent_limit_exceed                       
 )                        
 ...
 @ho_exrate_applied_type ,                        
 @sender_fax_no,@ID_Issue_date,@SSN_Card_ID,@Date_of_Birth,@Sender_State,                        
 @compliance_flag,@compliance_sys_msg,0,              
 @sPaymentReceivedType,@sCheque_bank,@sChequeno,@IssueAuthority,@payout_agent_id,@ben_bank_branch_id,@FreeSMS,
@PictureIdType,@customer_category_id,@agent_limit_exceed               
)                        
 set @tranno=@@identity                        
 ...
 BEGIN               
  ...
  select distinct              
   ...
   @tranno,              
   tcl.compliance_id                 
  FROM temp_compliance_log tcl WITH(NOLOCK) JOIN PaymentRule_Setup_v2 prsv WITH(NOLOCK)             
...
VALUES(@enc_refno)                        
 ...
 insert deposit_detail(bankcode,deposit_detail1,deposit_detail2,amtpaid,depositdot,tranno,          
pending_id)                        
 ...
 delete session_table where session_id=@session_id                         
  ...
  --exec spa_ComplianceCheck_Job_v2 @tranno,@transstatus              
...
if @@trancount>0                         
 ...
 INSERT INTO [error_info]                        
   ...
   ,[QueryString]                        
           ...
           ,[IP]                        
           ,[error_date])                        
 select -1,@desc,'spa_moneysend_main_V2','SQL',@desc,'SQL','SP',@ip_address,dbo.getDateHO(getutcdate())                        
