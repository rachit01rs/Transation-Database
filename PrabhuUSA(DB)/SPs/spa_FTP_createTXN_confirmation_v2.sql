IF OBJECT_ID('spa_FTP_createTXN_confirmation_v2','P') IS NOT NULL
DROP PROC [dbo].[spa_FTP_createTXN_confirmation_v2] 
Go
/*  
** Database    : PrabhuUSA  
** Object      : spa_FTP_createTXN_confirmation  
...
** Modifications  :  
			* added column ssn_card_id in monysend = futureuse of the (tbl_FTP_Import_File_Data) temp table.
			* and added with(lock) to the talbes. 	
*/
CREATE PROC [dbo].spa_FTP_createTXN_confirmation_v2 
@refno VARCHAR(50),
@FTP_processid VARCHAR(200),
@approve_by VARCHAR(50),
@CALC_BY CHAR(1) = 'p',  ---- 'c' by Collected AMT, 'p' Payout amt 
...
BEGIN TRY
	...
	--------------------------------------------------------------------------------------------------
	  SELECT 		@partnerID=t.PartnerID,
		  ...
		  WHERE     T.PINNO = @refno
					AND T.processid = @FTP_processid 
					AND T.DataInsertedInMoneySend='T' 
	
	IF @partnerID IS NULL 
	BEGIN
		SET @return_value = 'Invalid txn. Please contact Support Personal'    
	    SELECT '1001' Code,
	...
	IF ISNULL(@today_dollar_rate,0)=0 
	BEGIN
		SET @return_value = 'Invalid Exchange rate. Please contact Support Personal and resend this transaction.'    
	    SELECT '1001' Code,
	           @return_value MESSAGE	    
	    RETURN
	END
	...
	BEGIN
		SET @return_value = 'User not defined. Please to check the Setup.'    
	    SELECT '1001' Code,
	           @return_value MESSAGE	    
	    RETURN
	END
	...
	BEGIN
	    SET @return_value = 'Agent is Blocked'    
	    SELECT '1002' Code,
	           @return_value MESSAGE	    
	    RETURN
	END 
	...
	IF @Block_branch = 'y'
	BEGIN
	    SET @return_value = 'Branch is Blocked'    
	    SELECT '1003' Code,
	           @return_value MESSAGE	    
	    RETURN
	END 
...
--	END
	...
	SELECT @payout_agent_id = a.agentCode,
	       ...
	       @PAYOUTCURRENCY = ISNULL(@PAYOUTCURRENCY,currencyType),
	...
	BEGIN
	    SET @return_value = 'Payout Branch ID is not provided or Payout Branch ID doesnt matched with PayoutCountry'    
	    SELECT '1001' Code,
	           @return_value MESSAGE
	...
	BEGIN
	    SET @return_value = 'Invalid Payout Branch ID'    
	    SELECT '3003' Code,
	           @return_value MESSAGE
	...
	IF @branch_block = 'y'
	BEGIN
	    SET @return_value = 'Payout Branch ID is not active'    
	    SELECT '3004' Code,
	           @return_value MESSAGE
	...
	BEGIN
	    SET @return_value = 'Invalid Payout Branch ID and Country'    
	    SELECT '3003' Code,
	           @return_value MESSAGE	    
	    RETURN
	END    
	...
	BEGIN
	    ...
	    SET @OTHER_BANK_BRANCH_NAME = RTRIM(LTRIM(@OTHER_BANK_BRANCH_NAME)) 
	        ...
	        BEGIN
	        	 ...
	        	 select '3011' Code,@return_value Message
	        	 return
	        END
	...
	BEGIN
	    ...
	    BEGIN
	        SELECT '3008' Code,
	               'Select Country is not allowed, please contact Head Office' MESSAGE
	...
	IF @COLLECT_AMT <= 0
	BEGIN
	    SET @return_value = 'Amount is Invalid or Service Charge is not Defined.'    
	...
	BEGIN
	    SET @return_value = 'This agent don''t have sufficient balance to send txn'    
	    SELECT '3012' Code,
	           @return_value MESSAGE
	...
	BEGIN
	    SET @return_value = 'Dublicate txn Found'    
	    SELECT '9002' Code,
	           @return_value MESSAGE	    
	    RETURN
	END
	...
	SELECT 
	    ...
	    CASE WHEN @paymenttype = 'Account Deposit to Other Bank' AND @ifsc_code IS NOT NULL 
			AND PayoutCountry='NEPAL' THEN @ifsc_code END ,
	    t.futureUse
	 FROM [tbl_FTP_Import_File_Data] T WITH(NOLOCK)
	 WHERE  T.PINNO = @refno
			AND T.processid = @FTP_processid
...
-------------Update the txn in temp table for reference-------------------------------------------	
	 ...
	 SET    DataInsertedInMoneySend = 's' ,
			DataInsertedDate = GETDATE() ,
			Remarks = 'Ready for Payment.'
	 WHERE  DataInsertedInMoneySend='T'
			AND ProcessId = @FTP_processid
			AND PINNO = @refno
--------------------------------------------------------------------------------------------------	
	INSERT tbl_refno
	  (
...
BEGIN CATCH
	...
	SELECT -1,
	       @desc,
	       'spa_FTP_createTXN_confirmation',
	       'SQL',
	       @desc,
	       'SQL',
	       'SP',
