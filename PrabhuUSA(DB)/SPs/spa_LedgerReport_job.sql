/****** Object:  StoredProcedure [dbo].[spa_LedgerReport_job]    Script Date: 01/19/2015 14:31:29 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER proc [dbo].[spa_LedgerReport_job]                      
 @flag char(1),                      
 @agent_id varchar(50),                      
...
if @calc_currency='l' or @calc_currency is null                      
 ...
 begin                      
  ...
  FROM MoneySend with (nolock) where transStatus Not in(''Hold'') and agentid='+ @agent_id +'                      
 ...
 set @sql=@sql+' union all                        
  ...
  if @calc_currency='d' 
	        set @sql=@sql+' and isNull(ledger_effect_ccy,''d'')=''d''' 
  if @calc_currency='l' 
	        set @sql=@sql+' and isNull(ledger_effect_ccy,''l'')=''l''' 
 ...
 set @sql='insert #temp_arch1(Settlement_Amount)                      
  ...
  FROM MoneySend_arch1 with (nolock) where transStatus Not in (''Hold'') and agentid='+ @agent_id +'                      
  and confirmDate < '''+ @from_date +''''                      
 ...
 -- and b.agentcode ='''+@agent_id+'''                       
  ...
  if @calc_currency='d' 
	        set @sql=@sql+' and isNull(b.ledger_effect_ccy,''d'')=''d''' 
  if @calc_currency='l' 
	        set @sql=@sql+' and isNull(b.ledger_effect_ccy,''l'')=''l''' 
 ...
 begin                      
  ...
  case when mode=''cancel'' then ''Cancel TRN '' else '''' end +'' ''+ isNULL(b.remarks,'''') + '' - ''+                      
  case when mode=''receive'' or mode=''cr'' then '' ''+ isNULL(mt.moneyTransfer,'''')                       
                      
   else isNULL(convert(varchar(50),b.TRANNO),'''')+'' ''+ isNULL(mt.moneyTransfer,'''')                       
   end           
 ...
 set @sql=@sql+' end * -1)'                       
  ...
  FROM  AgentBalance b with (nolock) left outer join money_transfer mt  WITH(NOLOCK)                      
  on b.money_id=mt.money_id                      
  join agentdetail a WITH(NOLOCK) on a.agentcode=b.agentcode                      
  where b.mode not in (''FundPending'',''FundPendingDr'',''comdr'',''comcr'')  and b.approved_by is not null                      
  and b.agentcode='+ @agent_id +'                      
  ...
  if @branch_id is not null                      
   set @sql=@sql+' and b.branch_code='''+@branch_id +''''                      
  if @settlement_agent_id is not null                      
   set @sql=@sql+' and b.invoiceno in (                      
