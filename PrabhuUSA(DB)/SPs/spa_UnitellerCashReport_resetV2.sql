--spa_UnitellerCashReport_resetV2 @flag='u',@agent_id='92400000',@fromdate='2013-05-08'  
DROP PROC spa_UnitellerCashReport_resetV2
GO 
CREATE PROCEDURE [dbo].[spa_UnitellerCashReport_resetV2]      
 @flag CHAR(1) = NULL ,      
 @agent_id VARCHAR(50),      
...
FROM Account_Book_Close abc WHERE abc.book_type='UserCash' and abc.close_date<@fromDate ORDER BY close_ts DESC      
...
(isNUll(o.opening_balance,0)+isNUll(Collect_AMT,0) +isNUll(Transfer_IN,0)) -(isNUll(Paid_Amount,0)+isNUll(Transfer_Out,0)+ISNULL(Transfer_Out_Pending,0)) Balance INTO #Result FROM (  
...
UPDATE agentsub    
 SET current_balance=t.Balance    
 FROM agentsub s JOIN #Result t    
 ON s.User_login_Id=t.Login_USER_ID    
       
SELECT Login_USER_ID,Balance from #Result    
