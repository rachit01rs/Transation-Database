drop PROCEDURE [dbo].[spa_drildownreport]   
go 
CREATE PROCEDURE [dbo].[spa_drildownreport]          
 @flag char(1),          
...
SET @sql='SELECT CASE WHEN (GROUPING(companyname) = 1) THEN ''''           
 ...
 where isNull(a.non_IRH_Agent,''n'')=''n'' and '+ @dateType +' between '''+ @fromDate +''' and '''+ @toDate +' 23:59:59.997''          
 and Transstatus not in (''Hold'')'          
...
SET @sql='SELECT           
 ...
 where '+ @dateType +' between '''+ @fromDate +''' and '''+ @toDate +' 23:59:59.997''          
 and Transstatus not in (''Hold'')'          
  if @sendCountry IS NOT NULL          
 SET @sql= @sql+' and  sendercountry='''+ @sendCountry +''''          
if @senderAgent IS NOT NULL          
...
if @payoutAgent IS NOT NULL          
 SET @sql= @sql+' and expected_payoutagentid='''+ @payoutAgent +''''          
if @payoutBranch IS NOT NULL       SET @sql= @sql+' and rBankId='''+ @payoutBranch +''''          
if @statusType IS NOT NULL          
 SET @sql= @sql+' and status='''+ @statusType +''''          
if @paymentType IS NOT NULL          
 SET @sql= @sql+' and paymentType='''+ @paymentType +''''          
...
SET @sql= @sql+' GROUP BY a.companyname,pa.CompanyName,convert(varchar,'+@dateType+',102) WITH ROLLUP'          
--PRINT @sql          
EXEC (@sql)          
END          
IF @flag='b'          
