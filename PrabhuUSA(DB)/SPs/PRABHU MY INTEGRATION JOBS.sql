...
/****** Object:  Job [PRABHU MY INTEGRATION JOBS]    Script Date: 02/08/2012 17:39:35 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'PRABHU MY INTEGRATION JOBS', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Update TransStatus', 
		...
		@database_name=N'UAT_PrabhuCash', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
