DROP FUNCTION FNAExGetDenoRate
GO 
...
ELSE
BEGIN 
	SELECT @PaidCType=sellRate,@round_value=@round_value FROM dbo.Roster r WITH(NOLOCK) WHERE country=@sending_country_name
END
...
ELSE
BEGIN 
	SELECT @ReceiveCType=sellRate FROM dbo.Roster r WITH(NOLOCK) WHERE country=@receiving_country_name
END
